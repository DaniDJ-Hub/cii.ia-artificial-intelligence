...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    currentDprRef.current = Math.min(dpr ?? (window.devicePixelRatio || 1), 2);
    ...
    mount.style.backgroundColor = backgroundColor;
  ...
  useEffect(() => {
    ...
    const mount = mountRef.current;
    if (mount) mount.style.backgroundColor = backgroundColor;
  }, [
    wispDensity,
    mouseTiltStrength,
