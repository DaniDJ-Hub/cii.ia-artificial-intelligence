...
const DriftWall = ({
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      const tile = hit?.closest ? (hit.closest('[data-tile-id]') as HTMLElement | null) : null;
      if (!tile) return;
  ...
  const tileClass = cx(
    ...
    'h-[calc(var(--dw-tile-h)+var(--dw-gap))] w-full [transform-style:preserve-3d]',
  );
  const innerClass = cx(
    'pointer-events-none absolute inset-[calc(var(--dw-gap)/2)] block overflow-hidden bg-[var(--surface)]',
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Muro de proyectos"
    >
      <div ref={planeRef} className="absolute left-1/2 top-1/2 flex flex-row [transform-origin:50%_50%] [transform-style:preserve-3d] will-change-transform">
