...
const buildTextCanvas = (container: HTMLElement, width: number, height: number, dpr: number, props: RuntimeProps): HTMLCanvasElement => {
...
const WarpText = ({
  ...
  useEffect(() => {
    ...
    const rasterize = async () => {
      ...
      const textCanvas = buildTextCanvas(container, rect.width, rect.height, dpr, propsRef.current);
  ...
  return <div ref={containerRef} className={`relative isolate block min-h-[220px] w-full overflow-hidden ${className}`.trim()} style={style} role="img" aria-label={text} />;
};
