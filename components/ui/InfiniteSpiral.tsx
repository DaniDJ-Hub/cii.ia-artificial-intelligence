...
const InfiniteSpiral = ({
  ...
  const normalizedItems = useMemo<NormalizedItem[]>(
    () =>
      items.map((item, index) =>
        typeof item === 'string' ? { src: item, alt: `Elemento ${index + 1}` } : { alt: `Elemento ${index + 1}`, ...item },
      ),
    [items],
  );
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      const desiredAutoSpeed =
        autoEnabled && visibleRef.current && !reducedMotion.matches && !motionPaused ? speed * directionMultiplier : 0;
  ...
  const rootStyle: CSSProperties = {
    ...
    ['--spiral-width' as string]: `${cardWidth}px`,
    ['--spiral-height' as string]: `${cardHeight}px`,
    ['--spiral-radius' as string]: `${cardRadius}px`,
  ...
  const itemClassName =
    'absolute left-1/2 top-1/2 block h-[var(--spiral-height)] w-[var(--spiral-width)] overflow-hidden rounded-[var(--spiral-radius)] border border-white/15 bg-white/5 shadow-lg [backface-visibility:hidden] [transform-style:preserve-3d] [will-change:transform,opacity,filter]';
  ...
  return (
    ...
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" role="list" aria-label="Galería en espiral">
