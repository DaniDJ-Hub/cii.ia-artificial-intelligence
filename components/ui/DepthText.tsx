...
const DepthText = ({
  ...
  const rootStyle: CSSProperties = { ...style, perspective: `${safePerspective}px`, perspectiveOrigin: '50% 48%', contain: 'layout paint', isolation: 'isolate' };
  const stageStyle: CSSProperties = { transformStyle: 'preserve-3d', transform: getTransform(baseRotation.x, baseRotation.y), transformOrigin: '50% 50%', willChange: 'transform' };
  ...
  return (
    <span ref={rootRef} className={`inline-block ${className}`.trim()} style={rootStyle}>
      <span ref={stageRef} className="relative inline-grid place-items-center" style={stageStyle}>
        ...
        <span
          className="relative z-10 inline-block font-display"
          style={{
            ...textStyle,
            color: faceColor,
            textShadow: shadow ? `0 20px 32px color-mix(in srgb, ${depthColor} 36%, transparent), 0 4px 8px rgba(0,0,0,0.28)` : 'none',
